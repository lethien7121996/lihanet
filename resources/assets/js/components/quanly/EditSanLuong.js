...
class EditSanLuong extends Component {
        ...
        this.editor = <input type="text" defaultValue={this.state.text} onKeyPress={(event) => {
            ...
            if (key === 13) { //enter key
                ...
                var ttnext=Number(this.state.idtt)+1;
                    ...
                    document.getElementById('dl'+ttnext).click();
    ...
    handleUpdateSanLuong(value)
  {
     ...
     const sanluongupdate = {
      ...
      axios.post('/Demo/lihanet/index.php/api/sanluongupdate/'+this.state.idsanluong,sanluongupdate)
      .then(response => {
    ...
    render() {
        return this.state.editing ?
            this.editor
            : <p id={"dl"+this.state.idtt} onClick={this.edit}>{this.state.text}</p>
    }
}
export default EditSanLuong